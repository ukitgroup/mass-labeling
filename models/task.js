...
TaskSchema.statics = {
	...
	async getNew({ siteId, answer, userId }) {
		...
		if (limit && count >= limit) throw new Error('task_errors.markup_overdose');

		const site = await Site.findById(siteId);
		if (site.status === 'disabled') throw new Error('task_errors.bad_markup_request');
		if (site.status === 'approved' && answer === 0) throw new Error('task_errors.bad_markup_request');
